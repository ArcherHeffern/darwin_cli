use std::{fs::{create_dir, remove_dir_all, File, OpenOptions}, io, path::Path};
...
pub fn clean(darwin_path: &Path) -> io::Result<()> {
    ...
    OpenOptions::new().create(true).read(true).write(true).truncate(true).open(darwin_path.join("results").join("compile_errors"))?;
