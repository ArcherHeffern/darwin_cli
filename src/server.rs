...
use crate::{config::darwin_path, list_students::list_students, list_tests::list_tests, types::{TestResultError, TestResults}, view_student_results::parse_test_results};
...
fn get_student_result(student: &str, test: &str) -> Result<Json<TestResults>, status::Custom<String>> {
    match parse_test_results(darwin_path().as_path(), student, test) {
        Ok(r) => Ok(Json(r)),
        Err(e) => match e {
