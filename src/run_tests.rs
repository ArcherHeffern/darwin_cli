use std::{
    fs::{remove_dir_all, remove_file, rename, OpenOptions}, io::{self, prelude::*, Error, ErrorKind, Result}, path::{Path, PathBuf}, process::{Command, Stdio}
};
...
pub fn concurrent_run_tests(
    ...
    _concurrent_run_tests(darwin_path, test, num_threads, on_thread_start, on_thread_err, on_thread_end)
}
...
fn _concurrent_run_tests(
    ...
    for diff_path in darwin_path.join("submission_diffs").read_dir()? {
        ...
        threadpool.execute(move|| {
            ...
            match run_test_for_student(
                darwin_path_clone.as_path(),
                &student,
                &test_clone,
            ) {
                Ok(()) => {
...
pub fn run_test_for_student(darwin_path: &Path, student: &str, test: &str) -> Result<()> {
    ...
    if !darwin_path.is_dir() {
        return Err(Error::new(ErrorKind::AlreadyExists, "darwin project not initialized in this directory"));
    }
    if !is_test(darwin_path, test) {
        return Err(Error::new(ErrorKind::InvalidInput, format!("Test '{}' was not found", test)));
    }
    if !is_student(darwin_path, student) {
        return Err(Error::new(ErrorKind::InvalidInput, format!("Student '{}' was not found", student)));
    }
    ...
    // Don't recompute 
    let dest_file = darwin_path.join("results").join(format!("{}_{}", student, test));
    if dest_file.exists() {
        return Ok(());
    }
    
    _run_test_for_student(darwin_path, project_path.as_path(), student, test, dest_file.as_path())
}

fn _run_test_for_student(darwin_path: &Path, project_path: &Path, student: &str, test: &str, dest_file: &Path) -> Result<()> {
...
fn relocate_test_results(darwin_path: &Path, project_path: &Path, student: &str, test: &str) -> Result<()> {
