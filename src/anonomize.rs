...
use crate::{config::{compile_errors_file, student_diff_file, student_result_file}, list_students::list_students, list_tests::list_tests, util::file_replace_line};
...
fn anonomize_student(student: &str, i: usize, tests: &HashSet<String>) -> Result<()> {
    ...
    for test in tests {
        if student_result_file(student, test).is_file() {
            rename(student_result_file(student, test), student_result_file(&i.to_string(), test))?;
        }
    }
    rename(student_diff_file(student), student_diff_file(&i.to_string()))?;
